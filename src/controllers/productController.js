...
exports.getBySlug = (req, res, next) => {
  Product.findOne({active: true, slug: req.params.slug}, 'title description slug price tags').then(data => {
    res.status(200).send({data});
  }).catch(e => {
    res.status(400).send({e});
  });
...
exports.getByTag = (req, res, next) => {
  Product.findOne({active: true, tags: req.params.tag}, 'title description slug price tags').then(data => {
    res.status(200).send({data});
  }).catch(e => {
    res.status(400).send({e});
  });
...
exports.put = (req, res, next) => {
    ...
    }).then(x => res.status(200).send({message: "Produto atualizado com sucesso!"}))
